...
inductive BalTreeN (α : Type u) : ℕ → Type u where
  | leaf : α → BalTreeN α 0
  | node : {n : ℕ} → BalTreeN α n → BalTreeN α n → BalTreeN α (n + 1)
...
def root [Hash α] {n : ℕ} : BalTreeN α n → α
  | leaf a => a
  | node l r => l.root # r.root

section Root

@[simp] theorem root_leaf [Hash α] : (leaf a).root = a := rfl

@[simp] theorem root_node [Hash α] {l r : BalTreeN α n} :
    (node l r).root = l.root # r.root := rfl

end Root

def balTreeOfTuple : {n : ℕ} → (Fin (2^n) → α) → BalTreeN α n
  | 0, t => leaf (t 0)
  | (_ + 1), t =>
    let (f, l) := twoPowSuccTupleDivide t
    node (balTreeOfTuple f) (balTreeOfTuple l)

@[simp] theorem balTreeOfTuple_zero (bs : Fin (2^0) → α) :
  balTreeOfTuple bs = leaf (bs 0) := rfl

@[simp] theorem balTreeOfTuple_succ (bs : Fin (2^(n + 1)) → α) :
  balTreeOfTuple bs =
  node (balTreeOfTuple (twoPowSuccTupleDivide bs).1)
  (balTreeOfTuple (twoPowSuccTupleDivide bs).2) := rfl

def tupleOfBalTreeN {n : ℕ} : BalTreeN α n → (Fin (2^n) → α)
  | leaf a => fun _ => a
  | node l r => twoPowSuccTupleDivide.symm (l.tupleOfBalTreeN, r.tupleOfBalTreeN)

@[simp] theorem tupleOfBalTreeN_leaf : tupleOfBalTreeN (leaf a) = fun _ => a := rfl

@[simp] theorem tupleOfBalTreeN_node : tupleOfBalTreeN (node l r) =
  twoPowSuccTupleDivide.symm (l.tupleOfBalTreeN, r.tupleOfBalTreeN) := rfl

def equivBalTreeNTuple : BalTreeN α n ≃ (Fin (2^n) → α) where
  toFun := tupleOfBalTreeN
  invFun := balTreeOfTuple
  left_inv sww := by
    induction sww with | leaf _ => _ | node _ _ IHL IHR => _
    · rfl
    · simp_rw [tupleOfBalTreeN_node, balTreeOfTuple_succ,
        Equiv.apply_symm_apply, IHL, IHR]
  right_inv sww := by
    induction n with | zero => _ | succ _ IH => _
    · simp_rw [Nat.pow_zero, balTreeOfTuple_zero, tupleOfBalTreeN_leaf]
      exact funext fun _ => congrArg _ (Fin.fin_one_eq_zero _).symm
    · simp_rw [balTreeOfTuple_succ, tupleOfBalTreeN_node, Equiv.symm_apply_eq, IH]

instance : HMul (BalTreeN α n) (BalTreeN α n) (BalTreeN α (n + 1)) := ⟨node⟩

theorem root_mul [Hash α] {l r : BalTreeN α n}  :
    (l * r).root = l.root # r.root := rfl

end BalTreeN

def PSharedStack (α : Type u) (m : ℕ) := (i : Fin (m + 1)) → Option (BalTreeN α i)

open BalTreeN  in
def sngl : BalTreeN α 0 → PSharedStack α 0 := fun a _ => some _

def bit0 {n : ℕ} (p : PSharedStack α n) : PSharedStack α (n - 1) :=
  fun i => i.cases none (fun j => _)

/-
inductive PSharedStack (α : Type u) : ℕ → Type u
  | sngl : BalTreeN α 0 → PSharedStack α 0
  | bit0 : (n : ℕ) → PSharedStack α n → PSharedStack α (n + 1)
  | bit1 : {n : ℕ} → BalTreeN α (n + 1) → PSharedStack α n → PSharedStack α (n + 1)
deriving DecidableEq
-/

namespace PSharedStack
def get? : {n : ℕ} → PSharedStack α n → (m : ℕ) → Option (BalTreeN α m)
  | 0, sngl a, 0 => some a
  | 0, sngl _, (_ + 1) => none
  | n, bit0 n p, 0 => none
  | n, bit0 p, (m + 1) => _ -- _p.get? m
  | n, bit1 a p, 0 => _
  | n, bit1 _ p, (m + 1) => _ --p.get? m

end PSharedStack

@[reducible] def BalTree (α : Type*) := FreeMonoid ((n : ℕ) × BalTreeN α n)

namespace BalTree

inductive BalTreeRel (α : Type u) :
  BalTree α → BalTree α → Prop
| of_mul {n : ℕ} (l r : BalTreeN α n) :
  BalTreeRel α (FreeMonoid.of ⟨n, l⟩ * FreeMonoid.of ⟨n, r⟩)
    (FreeMonoid.of ⟨n + 1, l * r⟩)

inductive BalTreeRelClosure (α : Type u) :
  BalTree α →
  BalTree α → Prop
  | of : ∀ x y, BalTreeRel α x y → BalTreeRelClosure α x y
  | refl : ∀ x, BalTreeRelClosure α x x
  | symm : ∀ {x y}, BalTreeRelClosure α x y → BalTreeRelClosure α y x
  | mul : ∀ {w x y z}, BalTreeRelClosure α w x → BalTreeRelClosure α y z →
    BalTreeRelClosure α (w * y) (x * z)

end BalTree

open BalTree in
def BalTreeStack := Quot (BalTreeRelClosure α)

structure TreeHash (α : Type u) [Hash α] (n : ℕ) where
  hash : α
...
instance : HMul (TreeHash α n) (TreeHash α n) (TreeHash α (n + 1)) :=
  ⟨fun ⟨a⟩ ⟨b⟩ => ⟨a # b⟩⟩

def treeHashOfBalTreeN (t : BalTreeN α n) : TreeHash α n := ⟨t.root⟩

theorem treeHashOfBalTreeN_map_mul {l r : BalTreeN α n} :
    treeHashOfBalTreeN (l * r) = treeHashOfBalTreeN l * treeHashOfBalTreeN r := rfl
